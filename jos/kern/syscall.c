...
sys_exofork(void)
{
	...
	//set status to not runnable
	e->env_status = ENV_NOT_RUNNABLE;
	//copy parent's trapframe
	e->env_tf = curenv->env_tf;
	//set eax to zero to ret 0 in the child environment
...
sys_page_map(envid_t srcenvid, void *srcva, envid_t dstenvid, void *dstva, int perm)
{
	...
	if (envid2env(srcenvid, &e_src, 1) < 0)
		return -E_BAD_ENV;
	if (envid2env(dstenvid, &e_dst, 1) < 0)
		return -E_BAD_ENV;
	...
	pte_t* pte;
	struct PageInfo* page = page_lookup(e_src->env_pgdir, srcva, &pte);
	if (!page) return -E_INVAL;

	if (!(*pte & PTE_W) && (perm & PTE_W)) return -E_INVAL;
	
	if (page_insert(e_dst->env_pgdir, page, dstva, perm) < 0)
		return -E_NO_MEM;
