...
trap_init(void)
{
	...
	SETGATE(idt[T_DEFAULT], 0, GD_KT, trap_500, 0);
...
trap_init_percpu(void)
{
	...
	int cur_cpunum = cpunum();

	cur_cpu_ts->ts_esp0 = KSTACKTOP - cur_cpunum * (KSTKSIZE + KSTKGAP); 
   	cur_cpu_ts->ts_ss0 = GD_KD;
   	cur_cpu_ts->ts_iomb = sizeof(struct Taskstate);

   	size_t i = (GD_TSS0 >> 3) + cur_cpunum;
   	gdt[i] = SEG16(STS_T32A, (uint32_t)cur_cpu_ts, sizeof(struct Taskstate) - 1, 0);
   	gdt[i].sd_s = 0;

	ltr(GD_TSS0 + (cur_cpunum << 3));
   	lidt(&idt_pd);
}
...
page_fault_handler(struct Trapframe *tf)
{
	...
	// LAB 3: Your code here.
	if (!(tf->tf_cs & 3))
		panic("kernel page fault at va %08x", fault_va);
	...
	*/
	...
	if (curenv->env_pgfault_upcall) {
		...
		if ((tf->tf_esp < UXSTACKTOP) && (tf->tf_esp < UXSTACKTOP-PGSIZE)){
			ptr = tf->tf_esp - sizeof(uintptr_t);
		} else {
			ptr = UXSTACKTOP;	
		}
		ptr -= sizeof(struct UTrapframe);

		//u = (struct UTrapframe*)ptr; 
